...
const LEGENDARY_LINE_IDS = new Set(["star", "lava", "ancient", "crystal2"]);
const EGG_PITY_STEP = 0.05;
const EGG_PITY_CAP = 0.25;
...
function getEggDropChance({ difficulty = "easy", isBoss = false, isPerfect = false, eggPity = 0 } = {}) {
  const base = { easy: 0.35, normal: 0.45, hard: 0.6, hell: 0.75 }[difficulty] ?? 0.35;
  const bonus = (isBoss ? 0.15 : 0) + (isPerfect ? 0.1 : 0) + Math.min(EGG_PITY_CAP, Math.max(0, eggPity) * EGG_PITY_STEP);
  return Math.min(0.85, base + bonus);
}

function getTotalStarsFromSave(unitStars = {}) {
  return Object.values(unitStars || {}).reduce((sum, value) => sum + (Number(value) || 0), 0);
}

function isLockedLegendaryLine(lineId, totalStars) {
  return LEGENDARY_LINE_IDS.has(lineId) && totalStars < (PARTNER_UNLOCK_STARS[lineId] ?? Infinity);
}

function clearHatchSlot(slot) {
  return {
    ...slot,
    egg: null,
    startedAt: null,
...
function rebalanceEconomySave(saved) {
  if (!saved || saved.economyVersion === ECONOMY_VERSION) return saved;
  const totalStars = getTotalStarsFromSave(saved.unitStars);
  const coinCap = 600 + totalStars * 50 + (Number(saved.arenaWins) || 0) * 80;
  const shouldRemoveLine = (lineId) => isLockedLegendaryLine(lineId, totalStars);

  const monsterCollection = Object.fromEntries(
    Object.entries(saved.monsterCollection || {}).filter(([monsterId, entry]) => {
      const meta = getMonsterStageMeta(monsterId);
      const entryLineId = entry?.lineId || meta?.lineId;
      return !shouldRemoveLine(entryLineId);
    }).map(([monsterId, entry]) => [
      monsterId,
      {
        ...entry,
        lineExp: Math.min(300, Number(entry?.lineExp) || 0),
        evolutionCores: Math.min(1, Number(entry?.evolutionCores) || 0),
      },
    ])
  );

  const eggInventory = (saved.eggInventory || [])
    .filter((egg) => !shouldRemoveLine(egg?.lineId));
  const hatcherySlots = (saved.hatcherySlots || []).map((slot) => (
    shouldRemoveLine(slot?.egg?.lineId) ? clearHatchSlot(slot) : slot
  ));
  const activeLineRemoved = shouldRemoveLine(saved.lineId);

  return {
    ...saved,
    economyVersion: ECONOMY_VERSION,
    coins: Math.min(Number(saved.coins) || 0, coinCap),
    lineId: activeLineRemoved ? null : saved.lineId,
    stageIdx: activeLineRemoved ? 0 : saved.stageIdx,
    monLv: activeLineRemoved ? 1 : saved.monLv,
    monExp: activeLineRemoved ? 0 : saved.monExp,
    caughtMons: (saved.caughtMons || []).filter((monsterId) => {
      const meta = getMonsterStageMeta(monsterId);
      return !shouldRemoveLine(meta?.lineId);
    }),
    monsterCollection,
    eggInventory,
    hatcherySlots,
    pendingEggs: (saved.pendingEggs || []).filter((egg) => !shouldRemoveLine(egg?.lineId)),
    eggPity: Math.min(5, Math.max(0, Number(saved.eggPity) || 0)),
  };
}
...
function rollBattleObjective(wordsLength = 5) {
  const pool = [
    { id:"perfect", icon:"💎", title:"무결점 클리어", desc:"오답 없이 승리", rewardText:"+5G +4 라인EXP", bonusCoins:5, bonusExp:0, bonusLineExp:4 },
    { id:"combo3", icon:"🔥", title:"콤보 러시", desc:"3연속 정답 달성", rewardText:"+4G +5EXP", bonusCoins:4, bonusExp:5, bonusLineExp:0 },
    { id:"guard", icon:"🛡️", title:"체력 관리", desc:"HP 70% 이상으로 승리", rewardText:"+4G +4EXP", bonusCoins:4, bonusExp:4, bonusLineExp:0 },
  ];
  if (wordsLength >= 5) {
    pool.push({ id:"focus", icon:"⚡", title:"집중 폭발", desc:"FOCUS 스킬 1회 사용", rewardText:"+6G +5 라인EXP", bonusCoins:6, bonusExp:0, bonusLineExp:5 });
  }
...
export default function VocabMon() {
  ...
  const [eggPity, setEggPity] = useState(0);
  ...
  async function handleLogin(name, classCode) {
    let saved = await loadProgress(name, classCode);
    if (saved?.banned) return "banned";
    saved = rebalanceEconomySave(saved);
    ...
    if (saved) {
      ...
      setEggPity(Math.min(5, Math.max(0, Number(saved.eggPity) || 0)));
  ...
  const progressSnapshot = useMemo(() => ({
    ...
    lineId, stageIdx, curBook,
    streak, loginDays, lastLogin,
    caughtMons, pendingEggs, monsterCollection, eggInventory, hatcherySlots, eggPity,
  ...
  function endBattle(didWin, wc=wrongCount) {
    ...
    if(didWin){
      ...
      const ec = Math.round((10+curUnit*4) * diffMult) + bonusCoins;
      const ex = Math.round((22+curUnit*7) * diffMult) + bonusExp;
      setCoins(c=>c+ec);
      if(!dailyDone){ setDailyDone(true); }
      const levelResult = grantActiveMonsterExp(ex);
      const lineExpGain = Math.round((4 + stars * 3) * diffMult) + bonusLineExp;
      const coreGain = difficulty === "hell" && curSubStage === 4 && stars >= 3 ? 1 : 0;
      ...
      // 알 보상 + 일일 미션 업데이트
      const isPerfect = wc === 0;
      const isBoss = curSubStage === 4;
      const dropChance = getEggDropChance({ difficulty, isBoss, isPerfect, eggPity });
      const gotEgg = Math.random() < dropChance;
      const nextEggPity = gotEgg ? 0 : Math.min(5, eggPity + 1);
      let eggRarity = null;
      let pickedLine = null;
      if (gotEgg) {
        eggRarity = rollEggRarity({ difficulty, isBoss, isPerfect, totalStars });
        const possLines = EGG_DROP[eggRarity] || EGG_DROP.common;
        pickedLine = possLines[Math.floor(Math.random() * possLines.length)];
        // 시간 기반 부화 메타: 전투 보상 알은 인벤토리에 추가
        addEggToInventory(eggRarity, pickedLine, "unit_clear");
      }
      setEggPity(nextEggPity);
  ...
  useEffect(() => {
    ...
    qIdx, queue, phase, won, eggInventory.length, eggPity, readyEggCount, runningEggCount, dexProgress,
      ...
      <div data-testid="battle-screen" className="crt page slide-up" style={{background:"#0C0A18"}}>
        ...
        <div style={{position:"relative",flex:"0 0 auto",height:"clamp(160px,30vh,240px)",overflow:"hidden"}}>
          ...
          <div style={{
            ...
            zIndex: attackE ? spriteAttackZ : spriteBaseZ,
          ...
          <div style={{
            ...
            zIndex: attackP ? spriteAttackZ : spriteBaseZ,
            transformOrigin:"center bottom",
            animation: attackP
  ...
  if(screen==="result"&&mon) {
    ...
    const gotRewardEgg = Boolean(reward.gotEgg && reward.eggRarity && reward.eggLineId);
          ...
          <div className="reward-panel">
            ...
            <div className="reward-grid">
              ...
              <div className="reward-chip" style={{"--accent":rewardEggColor}}>
                <span>알 보상</span>
                <strong>{gotRewardEgg ? `${rewardEggLine?.eggEmoji || "🥚"} ${rewardEggLabel}` : "확률 상승"}</strong>
                <small>{gotRewardEgg ? (rewardEggLine?.name || "새 몬스터 후보") : `다음 알 확률 +${Math.min(25, (reward.eggPity || 0) * 5)}%`}</small>
              </div>
            </div>
            {reward.objective && (
...
//  SHOP SCREEN
  if(screen==="shop") {
    ...
    const hasLegendaryHatchSlot = hatcherySlots.some((slot) => slot.slotId === 3 && slot.unlocked);
    const canBuyLegendaryEgg = totalStars >= 45 && hasLegendaryHatchSlot;
    const ITEMS = [
      { id:"egg_common",   emoji:"🥚", name:"일반 알",      desc:"30분 · Common 라인 중심",       price:getEggRarityMeta("common").shopPrice, rarity:"common" },
      { id:"egg_rare",     emoji:"🥚", name:"레어 알",      desc:"2시간 · Rare 이상 확률 증가",    price:getEggRarityMeta("rare").shopPrice, rarity:"rare" },
      { id:"egg_sr",       emoji:"🌙", name:"슈퍼레어 알",  desc:"8시간 · Shadow/Dragon 계열",    price:getEggRarityMeta("superrare").shopPrice, rarity:"superrare" },
      { id:"egg_legend",   emoji:"✨", name:"레전드 알",    desc:canBuyLegendaryEgg ? "24시간 · 최상위 도감 라인" : "별 45개 + 전설 부화기 필요", price:getEggRarityMeta("legendary").shopPrice, rarity:"legendary" },
    ...
    function buyItem(item) {
      if (item.rarity === "legendary" && !canBuyLegendaryEgg) {
        setToast("레전드 알은 별 45개와 전설 부화기가 필요합니다.");
        return;
      }
      ...
      <div data-testid="shop-screen" className="crt page-y slide-up" style={{
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,flex:1}}>
          {ITEMS.map(item => {
            ...
            const isLegendDisabled = item.rarity === "legendary" && !canBuyLegendaryEgg;
            const isDisabled = isHatchDisabled || isSlotDisabled || isLegendDisabled;
            return (
